                ...
                for trigger_ in item_["col_structure"]["triggers"]:
                    for cluster_ in trigger_["targets"]:
                        self.triggers_.append({
                            ...
                            "conditions": trigger_["conditions"],
    ...
    async def worker_f(self, event_, resume_token_):
        ...
        try:
            ...
            if source_properties_ is None:
                raise AppException(f"no source properties defined for '{source_collection_id_}'")
            ...
            trigger_targets_ = [tg_ for tg_ in self.triggers_ if tg_["source"] == source_collection_id_ and op_ in tg_["operations"] and [ma_ for ma_ in tg_["changes"] if ma_["key"] in changed_keys_ and (
                ...
                (ma_["op"].lower() == "lte" and changed_[ma_["key"]] <= ma_["value"])
            )]]
            if trigger_targets_ == []:
                raise PassException("!!! no trigger found")
            ...
            for target_ in trigger_targets_:
                ...
                conditions_match_ = target_["conditions"] if "conditions" in target_ and len(target_["conditions"]) > 0 else None
                if conditions_match_:
                    arr_ = [cm_ for cm_ in conditions_match_ if cm_["key"] not in [tc_["key"] for tc_ in target_changes_]]
                    if len(arr_) > 0:
                ...
                # print(">>> source conditions okay", match_, full_document_)
                ...
                filter_ = {}
                target_filters_ = target_["filter"]
                ...
                set_ = {}
                sets_ = target_["set"]
                upsert_ = target_["upsert"] if "upsert" in target_ else False
                for item_ in sets_:
                    ...
                    target_enum_ = True if target_field_ in target_properties_ and "enum" in target_properties_[target_field_] and len(target_properties_[target_field_]["enum"]) > 0 else False
                    ...
                    # print(f">>> type22_", type_)
                    # print(f">>> target_bson_type_", target_bson_type_)
                    if type_ == "formula":

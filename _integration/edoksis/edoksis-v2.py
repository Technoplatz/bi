...
def issue_f():
    ...
    try:
        ...
        for delivery_ in deliveries_:
            ...
            delivery_line_no_ = delivery_["dnn_line_no"]
            delivery_product_no_ = delivery_["dnn_prd_no"]
            delivery_product_desc_ = delivery_["dnn_prd_description"]
            item_name_ = f"{delivery_no_}-{delivery_line_no_} {delivery_product_no_} {delivery_product_desc_}"
        ...
        session_db_["shipment_data"].update_one({"shp_id": shipment_id_}, {"$set": set_}, upsert=True)
        ...
        files_ = []
        # if not err_:
        #     get_waybill_f_ = Edoksis().get_waybill_f([shipment_id_])
        #     if not get_waybill_f_["result"]:
        #         raise APIError(get_waybill_f_["msg"])

...
export class CollectionPage implements OnInit {
  ...
  constructor() {
    this.crud.collections.subscribe((res: any) => {
      ...
      this.title_ = this.collections_?.find(
        (obj_: any) => obj_.col_id === this.subheader,
      )?.col_title;
    });
  ...
  get_links(res: any) {
    return new Promise((resolve, reject) => {
      this.links_ = res.structure.links
              ...
              this.misc
                ...
                .then((res: any) => {
                  ...
                  this.get_links(res).then(() => {
                    ...
                    this.scan_ = true
                      ? Object.keys(this.properties_).filter(
                          (key: any) => this.properties_[key].scan,
                        ).length > 0
                      : false;
  ...
  init_search(full: boolean) {
    ...
    this.storage.set('LSFILTER_' + this.id, this.filter_).then(() => {
      if (this.searched) {
        for (let key_ in this.structure_.properties) {
          this.searched[key_] = full
            ? { actived: false, kw: null, f: false, op: 'contains' }
            : {
                actived: false,
                kw: this.searched[key_]?.kw ? this.searched[key_]?.kw : null,
                f: this.searched[key_]?.f,
  ...
  copy(cpd_: any, event_: any, indx_: number) {
    ...
    indx_ >= 0 ? (this.data[indx_].is_copied = false) : null;
    this.misc
      .copy_to_clipboard(cpd_)
      .then(() => {
        this.is_copied = true;
        indx_ >= 0 ? (this.data[indx_].is_copied = true) : null;
      })
      .catch((error: any) => {

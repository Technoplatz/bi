...
export class Crud {
  ...
  init_form(op: string, structure: any, form: any, data: any, collections: any, counters: any, actionix_: number) {
    return new Promise((resolve) => {
      ...
      for (const item of names_) {
        ...
        const v = [];
        if (required_) { v.push(Validators.required); }
        if (minLength_ && bsonType_ !== "array") { v.push(Validators.minLength(minLength_)); }
        if (maxLength_ && bsonType_ !== "array") { v.push(Validators.maxLength(maxLength_)); }
        if (minimum_) { v.push(Validators.min(minimum_)); }
        if (maximum_) { v.push(Validators.max(maximum_)); }
        if (pattern_ && !p.file) { v.push(Validators.pattern(pattern_)); }
        this.fields.push({
          name: item, title: p.title, enum: enums_, required: required_, bsonType: bsonType_,
          description: p.description ? p.description : null, password: !!p.password, maxLength: maxLength_, minLength: minLength_,
          minItems: p.minItems && p.minItems >= 0 ? p.minItems : null, maxItems: p.maxItems && p.maxItems >= 0 ? p.maxItems : null,
          parents: parents_, items: p.items ? p.items : null, file: !!p.file, filter: !!p.filter, kv: kv_, ko: p.subType === "keyop",
          permanent: permanent_, readonly: !!p.readonly, collection: collection_, view: !!p.view, property: !!p.property,
          textarea: !!p.textarea, decimals: p.decimals && p.decimals > 0 ? p.decimals : null, hashtag: !!p.hashtag, chips: !!p.chips,
          subType: p.subType ? p.subType : null, manualAdd: !!p.manualAdd, placeholder: p.placeholder ? p.placeholder : null,
          counter: p.counter === true, dateOnly: !!p.dateOnly, caseType: p.caseType ? p.caseType : null, timestamp: timestamp_,
          selection: !!p.selection, reminder: !!p.reminder
        });
        const kvval_: any = [{ key: null, value: null }];
        init[item] = timestamp_ ? new Date(Date.now() - tzoffset).toISOString().replace(/-|:|T/gi, "").substring(0, 14)
          : counters && counters[item] ? counters[item] : default_ ? default_ : arrayInc_ ? kv_ ? kvval_ : [] : objectInc_ ? {} : null;
        form.addControl(item, new UntypedFormControl({
          value: data && data[item] ? data[item] : init[item],
          disabled: disabled_ || (item === "id" && op === "update" && (data[item] || init[item])) ? true : false
    ...
    return new Promise((resolve, reject) => {
      ...
      if (file) {
        ...
        this.misc.api_call("crud", {
          op: op, collection: collection, doc: doc_, data: data_,
          match: match && match.length > 0 ? match : null, filter: filter ? filter : null, _id: _id ? _id : null,
          actionix: actionix, link: link_, linked: linked_
        }).then((res_: any) => resolve(res_)).catch((error_: any) => reject(error_));
      }
    });
  }

  get_collections() {
    return this.misc.api_call("crud", { op: "collections", collection: "_collection" }).then((res: any) => {
  ...
  get_query_job(type_: string, id_: string, limit_: number, run_: boolean) {
    return this.misc.api_call("crud", { id: id_, op: type_, collection: type_ === "job" ? "_job" : "_query", limit: limit_, run: run_ });
  }

  announce(id_: string, type_: string, otp_: number) {
    return this.misc.api_call("crud", { id: id_, op: "announce", collection: "_query", tfac: otp_, type: type_ });
  }
  ...
  get_all() {
    return this.get_collections().catch((error: any) => {

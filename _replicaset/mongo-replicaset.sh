...
if [ ! -f /init/mongo-init.flag ]; then
    mongosh "mongodb://$MONGO_HOST:$MONGO_PORT/?authSource=$MONGO_AUTH_DB&tlsInsecure=$MONGO_TLS_INSECURE" --quiet --tls --tlsCertificateKeyFile /init/$MONGO_HOST.pem --tlsCAFile $MONGO_TLS_CA_COMBINED_FILE --eval "
        rs_ = {
...
until [[ $RS_OK -eq "1" ]]; do
    ...
    RS_STATUS=$(mongosh "mongodb://$MONGO_HOST:$MONGO_PORT/?authSource=$MONGO_AUTH_DB&tlsInsecure=$MONGO_TLS_INSECURE" --quiet --tls --tlsCertificateKeyFile /init/$MONGO_HOST.pem --tlsCAFile $MONGO_TLS_CA_COMBINED_FILE --eval "
...
MONGO_INDEXOF_DB=$(mongosh "mongodb://$MONGO_HOST:$MONGO_PORT/?authSource=$MONGO_AUTH_DB&tlsInsecure=$MONGO_TLS_INSECURE" --quiet --tls --tlsCertificateKeyFile /init/$MONGO_HOST.pem --tlsCAFile $MONGO_TLS_CA_COMBINED_FILE --eval "
...
if [[ $MONGO_INDEXOF_DB -eq "-1" ]]; then
    ...
    mongosh "mongodb://$MONGO_HOST:$MONGO_PORT,$MONGO_REPLICA1_HOST:$MONGO_PORT,$MONGO_REPLICA2_HOST:$MONGO_PORT/?replicaSet=$MONGO_REPLICASET&authSource=$MONGO_AUTH_DB&tlsInsecure=$MONGO_TLS_INSECURE" --quiet --tls --tlsCertificateKeyFile /init/$MONGO_HOST.pem --tlsCAFile $MONGO_TLS_CA_COMBINED_FILE --eval "
